...
class Cart:
    ...
    def __iter__(self):
        ...
        for item in cart.values():
            ...
            item['unique_id'] = self.unicode_id_generator(product.id, item['color'], item['size'])
            yield item

    def unicode_id_generator(self, id, color, size):
    ...
    def add(self, product, quantity, color, size):
        unique = self.unicode_id_generator(product.id, color, size)
